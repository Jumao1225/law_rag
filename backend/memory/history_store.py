...
def _build_light_model():
    ...
    try:
        # model: 实际调用的轻量模型名称，例如 deepseek-chat。
        ...
        return ChatOpenAI(model=config.light_model_name, api_key=api_key, base_url=base_url)
...
def _summarize_messages(
    ...
    try:
        ...
        logger.warning("摘要模型调用失败，进入 fallback 摘要逻辑")
...
def _compress_messages(all_messages: list[BaseMessage]) -> list[BaseMessage]:
    ...
    if not all_messages:
        logger.debug("输入为空，跳过压缩")
        return []

    input_tokens = _estimate_messages_tokens(all_messages)
    logger.debug(f"开始压缩: 输入消息={len(all_messages)}条, 估算token={input_tokens}")
